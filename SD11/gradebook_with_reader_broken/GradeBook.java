...
public class GradeBook {
    ...
    public GradeBook(InputStream stream, int passedMaxNumberStudents, int passedMaxAssignments) {
        this("", passedMaxNumberStudents, passedMaxAssignments);
        Scanner scanner = new Scanner(stream);
        className = scanner.nextLine();
        scanner.useDelimiter(Pattern.compile("\\p{javaWhitespace}*,\\p{javaWhitespace}*|\\n"));
        // throw away Student Name
        scanner.next();
        int numAssignments = 0;
        String [] tempAssignmentNamesArray = new String [passedMaxAssignments];
        String assignmentName = scanner.next();
        while(assignmentName.equals("Total") == false) {
            tempAssignmentNamesArray[numAssignments] = assignmentName;
        ...
        scanner.next();
        int counter = 0;
        String assignmentWeightAsString = scanner.next();
        while(assignmentWeightAsString.equals("Total") == false) {
            addAssignment(tempAssignmentNamesArray[counter], Double.parseDouble(assignmentWeightAsString));
            counter = counter + 1;
            assignmentWeightAsString = scanner.next();
        }
        while(scanner.hasNext() == true) {
            // read the name and create that student
